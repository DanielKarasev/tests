...
class FRPListPageLocators:
    SUBJECT_COLUMN = "span.ant-table-column-title:text-is('Предмет')"
    BASE_LEVEL_COLUMN = "span.ant-table-column-title:text-is('Базовый уровень')"
    ADD_ED_LEVEL_FIELD = "input#educationLevelId"
    ADD_COURSE_FIELD = "input#subjectId"
    BASE_LEVEL_CHECKBOX = "#isBasic[type='checkbox']"
    FILE_NAME = f"div:text-is('{props['frp_test_name']}')"
    BASE_NAME = f"div:text-is('{props['frp_test_base_name']}')"
    BASE_CHECK = "xpath=//table/tbody/tr[2]/td[7]/div"
    CHANGED_COURSE_NAME = f"div:text-is('{props['courses_new_test_name']}')"
    NEW_FILE_NAME = f"div:text-is('{props['frp_new_test_name']}')"
    NEW_BASE_NAME = f"div:text-is('{props['frp_new_test_base_name']}')"

class FRPPageLocators:
    TEST_NAME = f"div:text-is('{props['frp_test_name']}')"
    COUNT_100_BUTTON = "span:text-is('100')"
    NUMBER_OF_DOCUMENTS = "span.ant-typography"
    CLEAR_CHOICE_FIELD = ".ant-select-clear"
    ED_LEVEL_CHOICE_FIELD = "#root #rc_select_0"
    ED_LEVEL_CHOICE_FIELD_NOO = "#root [title='Начальное общее образование']"
    ED_LEVEL_CHOICE_FIELD_OOO = "#root [title='Основное общее образование']"
    COURSE_CHOICE_FIELD = "#root #rc_select_1"
    COURSE_LEVEL_CHOICE_FIELD = "#root #rc_select_2"
    COURSE_LEVEL_CHOICE_FIELD_BASE_LEVEL = "#root [title='Базовый уровень']"
    ED_LEVEL_NOO = "div[title='Начальное общее образование']"
    ED_LEVEL_OOO = "div[title='Основное общее образование']"
    ED_LEVEL_SOO = "div[title='Среднее общее образование']"
    TEST_COURSE = f"div[title='{props['courses_test_name']}']"
    COURSE_LEVEL_BASE = "div[title='Базовый уровень']"
    COURSE_LEVEL_DEEP = "div[title='Углубленный уровень']"

class FRPSitePageLocators:
    COURSE_LINE = "div:text-is('Предмет')"
    BASE_LEVEL_LINE = "div:text-is('Базовый уровень')"
    COURSE = "div:text-is('Предмет') + div"
    BASE_LEVEL_CONDITION = "div:text-is('Базовый уровень') + div"